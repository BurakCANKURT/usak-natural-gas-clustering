...
class ClusteringUsak:
    ...
    def KMeansFit_predict(self,X_pca,filter_name):
        ...
        fig, ax = plt.subplots()
        scatter = ax.scatter(X_pca[:, 0], X_pca[:, 1], c=clusters, cmap="viridis")  # Scatter objesi
    ...
    def AggFit_predict(self, sample_df, X_pca, filter_name):
        ...
        fig, ax = plt.subplots()
        scatter = ax.scatter(X_pca[:, 0], X_pca[:, 1], c=agg_cluster, cmap="viridis")  # scatter objesi
    ...
    def find_outliers(self):
        ...
        """
        #0.05 ---> 0.5 Still has outliers Not effective
        iso = IsolationForest(contamination=0.55)
        df["anomaly_iso"] = iso.fit_predict(scaled_X) == -1
        """
        self.df = self.df[self.df["anomaly_z"] == False]


        fig, ax = plt.subplots(figsize=(10, 6))  # ✔️ Doğru kullanım
    ...
    def  geospatial_consumption_profiling(self):
        self.find_outliers()

        m = folium.Map(location=[self.df["latitude"].mean(), self.df["longitude"].mean()], zoom_start=10)
